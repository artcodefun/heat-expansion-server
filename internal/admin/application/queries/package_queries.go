...
func (q *PackageQueries) ListCrystalPackages(ctx context.Context, _ cqrs.Actor) ([]*readmodels.CrystalPackage, error) {
	list, err := q.billing.ListCrystalPackages(ctx)
	return list, clientErr(err)
}

func (q *PackageQueries) GetCrystalPackage(ctx context.Context, _ cqrs.Actor, id uuid.UUID) (*readmodels.CrystalPackage, error) {
